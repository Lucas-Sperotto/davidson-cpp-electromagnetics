...
As observações abaixo já incorporam, quando disponível, a validação numérica recente contra os scripts MATLAB originais.
...
| --- | --- | --- | --- |
| `fdtd_1D_demo.m` | `Cap_02/src/fdtd_1D_demo.cpp` | Traduzido | Demo principal do capítulo, realinhado com o MATLAB e validado numericamente |
| `fdtd_1D_WB_demo.m` | `Cap_02/src/fdtd_1D_WB_demo.cpp` | Traduzido | Demo banda larga validada no domínio do tempo e da frequência |
| `gaussder_norm.m` | `Cap_02/src/gaussder.cpp` / `Cap_02/include/gaussder.hpp` | Traduzido | Função isolada como módulo próprio e conferida pelo uso nos dois demos |
...
| `FDTD_2D/fdtd_2d_pml_demo.m` | `Cap_03/src/fdtd2d_pml.cpp` | Traduzido com adaptação | Validado contra o MATLAB por histórico temporal e snapshots internos, inclusive no regime tardio instável herdado do original |
...
| MATLAB original | Equivalente atual | Status | Observação |
| --- | --- | --- | --- |
| `MoM_2D_TM.m` | `Cap_04/MoM_2D_TM.cpp` | Traduzido com adaptação | Validado contra o MATLAB nos ramos de corrente e RCS; o sweep físico coincide bem, com pequena divergência em colunas auxiliares de bookkeeping |
...
| `static_mom.m` | `Cap_04/static_mom.cpp` | Traduzido com adaptação | Script convertido para executável com saída CSV e validado numericamente contra o MATLAB |
| `thin_dipole.m` | `Cap_04/thin_dipole.cpp` | Traduzido com adaptação | Entradas interativas convertidas para CLI e saídas CSV; correntes validadas contra o MATLAB após alinhar o eixo `z` |

## Capítulo 6

| MATLAB original | Equivalente atual | Status | Observação |
| --- | --- | --- | --- |
| `Mixed Potential EFIE RWG/MoM3D_demo.m` | `Cap_06/MoM3D_demo.cpp` | Traduzido com adaptação | Prompt interativo substituído por CLI e saídas CSV; correntes e métricas principais validadas contra o MATLAB |
...
| `Sphere RCS/converge_sphereRCS.m` | `Cap_06/converge_sphereRCS.cpp` | Traduzido com adaptação | Script convertido para executável com saída CSV e validado contra as curvas MATLAB |
...
| MATLAB original | Equivalente atual | Status | Observação |
| --- | --- | --- | --- |
| `scalar_pot.m` | `Cap_07/scalar_pot.cpp` | Traduzido com adaptação | Gera CSVs para as figuras das integrais de Sommerfeld; validado contra o MATLAB com ressalva intencional no primeiro ponto exportado da `region3` |
| `V_pot_eps.m` | `Cap_07/V_pot_eps.cpp` | Traduzido | Estudo do potencial para diferentes permissividades, validado contra o MATLAB |
| `V_pot_height.m` | `Cap_07/V_pot_height.cpp` | Traduzido | Estudo do potencial para diferentes alturas, validado contra o MATLAB |
| `MoM_Som.m` | `Cap_07/MoM_Som.cpp` | Traduzido com adaptação | Prompt interativo substituído por CLI e impedância validada contra o MATLAB |
...
| MATLAB original | Equivalente atual | Status | Observação |
| --- | --- | --- | --- |
| `FEM_1D_1st.m` | `Cap_09/FEM_1D_1st.cpp` | Traduzido | Programa principal validado contra o MATLAB por perfis e curva de convergência |
...
| MATLAB original | Equivalente atual | Status | Observação |
| --- | --- | --- | --- |
| `Static2D.m` | `Cap_10/Static2D.cpp` | Traduzido | Solver principal validado contra o MATLAB em capacitância, potencial e campos |
| `Eigen2D.m` | `Cap_10/Eigen2D.cpp` | Traduzido | Solver principal com Whitney validado contra o MATLAB |
...
| MATLAB original | Equivalente atual | Status | Observação |
| --- | --- | --- | --- |
| `Eigen3D_v0.m` | `Cap_11/Eigen3D_v0.cpp` | Traduzido com adaptação | Validado contra helper compatível usando a mesma decomposição determinística de cada brick em 6 tetraedros adotada no C++ |
| `Eigen3D_CTLN.m` | `Cap_11/Eigen3D_CTLN.cpp` | Traduzido com adaptação | Suporta malha interna e leitura Gmsh; validado contra o MATLAB com `box_20.msh` |
| `Eigen3D_LTQN.m` | `Cap_11/Eigen3D_LTQN.cpp` | Traduzido com adaptação | CLI substitui prompts e suporta ordem 1 e 2; validado contra o MATLAB com `box_20.msh` |
| `FETD_FDTD.m` | `Cap_11/FETD_FDTD.cpp` | Traduzido com adaptação | Saída em CSV no lugar do plot direto e validada contra o MATLAB |
...
| `test_tet_quad.m` | `Cap_11/test_tet_quad.cpp` | Traduzido com adaptação | Resultado exportado em CSV e validado em precisão de máquina |
