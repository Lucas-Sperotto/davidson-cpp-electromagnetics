...
// Definir essas variáveis como globais ou encapsuladas num contexto adequado
...
std::vector<std::vector<int>> LOCALEDGENODES = {
    {0, 1}, // edge 1: nodes 1-2
    {0, 2}, // edge 2: nodes 1-3
    {1, 2}  // edge 3: nodes 2-3
};
...
void Eigen2D_LTQN() {
    ...
    void finalize_LTQN(double a, double b,
        ...
        int num_free_edges = std::count_if(edge_free_flag.begin(), edge_free_flag.end(), [](int f) { return f == 1; });
        int num_zero_eigvals = num_free_nodes + num_free_edges;
    
        // Avaliação de erro dos primeiros modos úteis
        evaluate_TE_error(a, b, indexed_kc, std::min(NUM_DOFS - num_zero_eigvals, 8), num_zero_eigvals);
